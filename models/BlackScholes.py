...
class BlackScholesOption:
    ...
    def call_price(self):
        call_price = self.spot_price * norm.cdf(self.d1()) - self.strike_price * np.exp(
            -self.risk_free_rate * self.time_to_expiry
        ) * norm.cdf(self.d2())
        return call_price

    def put_price(self):
        put_price = self.strike_price * np.exp(
            -self.risk_free_rate * self.time_to_expiry
        ) * norm.cdf(-self.d2()) - self.spot_price * norm.cdf(-self.d1())
        return put_price
...
class OptionSimulator:
    ...
    def simulate(self):
        # Simulate option prices for all combinations of input parameters
        option_prices = {
            "price": [],
            "S": [],
            "K": [],
                ...
                for k, time in enumerate(self.time_to_expiry):
                    for l, sigma in enumerate(self.volatility):
                        for m, rate in enumerate(self.risk_free_rate):
                            ...
                            option_prices["call_option"].append(
                                1 if self.call_option else 0
                            )
